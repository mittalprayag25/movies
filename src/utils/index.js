...
const memoize = (method, { path = [], timeout = Infinity } = {}) => {
  ...
  return function () {
    ...
    const args = R.when(
      ...
      (argsList) =>
        R.length(argsList) > 0 &&
        R.both(R.is(Object), R.has('memoize'))(R.last(argsList)),
      (argsList) =>
        ({
          memoize: { renew },
    ...
    invoke &&
      do {
        ...
        timeout !== Infinity &&
          setTimeout(() => (fresh[cachedArgs] = false), timeout);
      };
...
const toObjBy = R.curry((name, arr) => R.pipe(R.groupBy(R.prop(name)), R.map(R.prop(0)), R.omit(['undefined']))(arr));
